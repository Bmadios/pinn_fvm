...
python main.py --data_path post_process_data.csv
...
def main(args):
    ...
    N_mes = 10  # number of measurements points
    ...
    if args.pretrained_model is None:
        ...
        with open('training_log.csv', mode='w', newline='') as csvfile:
            ...
            while (epoch <= max_epochs and loss.item() > 8.5e-7):
                # Perturbation
                if epoch % 5 == 0:
                    XY_pde = colloc_pde_with_mesh(N_x, N_y, x_min, x_max, y_min, y_max, x_center, y_center, r_cyl)
                    XY_in = colloc_Xinlet(N_bc, x_min, y_min, y_max)
                ...
                log_writer.writerow([epoch, loss.item(), args.learning_rate])

                if epoch % 500 == 0:
                    #current_learning_rate = optimizer.param_groups[0]['lr']
                    #print(f'epoch: {epoch}, loss_pde: {loss.item()}, total_loss:{loss_hard.item()}, lr = {args.learning_rate}', flush=True)
                    print(f'epoch: {epoch} / {max_epochs}, loss_pde: {loss.item()}, lr = {args.learning_rate}', flush=True)
